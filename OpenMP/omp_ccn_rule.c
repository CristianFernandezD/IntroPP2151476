...
/*
  ...
  Modificación del método:
    ...
    Este método se ha modificado para que las variables N, A, B y 
    FILENAME sean declaradas con los siguientes parametros 
    especificos: N = 9, A = -1, B = 1 y FILENAME = "ccn_o9"; 
    los cuales han sido tomados del ejemplo de la salida del 
    algoritmo ccn_rule mostrado en el siguiente enlace: 
    https://people.sc.fsu.edu/~jburkardt/c_src/ccn_rule_test/
    ccn_rule_test.txt
    
    Otra modificación que se ha hecho al método, es la construcción
    de un código para determinar el tiempo que tarda en ejecutar el
    algoritmo. Este código fue tomado en base al del enlace, aunque
    se tuvo que mejorar algunos errores: https://foro.elhacker.net/
    programacion_cc/medir_el_tiempo_de_ejecucion_en_c-t462042.0.html
...
{  
  ...
  printf ( "\n" );
/*
  El usuario ya no especifica los valores iniciales.
*/
  printf ( "  Los valores N, A, B y FILENAME fueron asignados para facilitar el análisis.\n" );
  printf ( "\n" );
...
*/
  ...
  else
  {
    a = -1;    
    printf ( "\n" );
    printf ( "  El punto final izquierdo (A) es: %g\n", a );
  }
/*
  Get B.
*/
  ...
  else
  {
    b = 1;    
    printf ( "\n" );
    printf ( "  El punto final derecho (B) es: %g\n", b );
  }
/*
  Get FILENAME:
*/
  ...
  {
    strcpy ( filename, "ccn_o9" );    
    printf ( "\n" );
    printf ( "  El nombre raiz de los archivos de cuadratura (FILENAME) es: \"%s\".\n", filename );
  }
/*
  Input summary.
*/
...
/*
  Esto pone en pausa la salida del algoritmo para poder observar mejor la compilación en el sistema local.
*/
  system("PAUSE");
...
/*
  ...
  Modificación de OpenMP:
  
    02 de Julio 2020 por Cristian Fernandez, Universidad Industrial de
    Santander.
    Este algoritmo ha sido modificado con OpenMP para permitir la
    paralelización del código original.
...
*/
/*
  Se paraleliza el ciclo para determinar su rendimiento.
*/
  #pragma omp parallel shared(x, r8_pi) private(i)
  {
          #pragma omp for
          for ( i = 0; i < n; i++ ) 
          {
...
/*
  Se paraleliza el ciclo mas externo.
*/
  #pragma omp parallel shared(d, w) private(i, j, k, x_min, x_max)
  {
...
*/
/*
  Paralelización y variables
*/
  #pragma omp parallel shared(output, table)  private(i, j)
  {
          #pragma omp for
          for ( j = 0; j < n; j++ )
          {
...
/*
  Se paraleliza el siguiente ciclo.
*/
  #pragma omp parallel
  {
          #pragma omp for
